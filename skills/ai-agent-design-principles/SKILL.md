...
Production-tested principles for building AI agents that feel natural, intelligent, and trustworthy. Derived from analysis of shipping-grade system prompts. These are not theoretical — they are patterns that work at scale with real users.

**Core insight:** The difference between a mediocre AI agent and a great one is not capability — it's *restraint*. Knowing when NOT to format, NOT to personalize, NOT to over-explain is what makes interactions feel human.

## When to Use

- Designing or reviewing system prompts for any AI agent
- Building chatbots, coding assistants, customer support bots, internal tools
- Agent feels "robotic" — over-formatted, over-eager, formulaic
- Agent leaks internal reasoning ("I see in my data...", "Based on my memory...")
- Agent applies user preferences/context inappropriately (wine metaphors in Python questions)
- Building memory, preference, or tool-suggestion systems
- Writing interaction guidelines for AI products

## When NOT to Use

- Pure code generation tasks with no user interaction
- Batch processing or API-only pipelines
- Single-turn factual Q&A with no personalization needs
- Designing characters or personas for creative writing (where distinct, non-neutral personalities are desired)
- Building aggressive red-teaming or adversarial agents
...
- Match response length to query complexity — don't pad simple answers
- One-liner questions get one-liner answers
- Complex questions get proportional depth, but never filler
- Never add content just to appear thorough
...
| Evenhandedness | Present multiple perspectives. Don't take political sides. |
| Response length | Match complexity. Never pad. |
| Sycophancy | No "Certainly!", "Great question!". Just answer. |
| Cultural context | Adapt principles for cultural audience. |
...
| "Would you like to use our amazing X tool?" | "Oh, I can actually do that for you" |
| Excessive apologies for errors | Acknowledge, fix, move on |
| "I'm always here for you" | Don't foster dependency |
| Explaining why a request was filtered | State the principle, hide the mechanics |
| "Certainly! Great question!" openers | Just start answering |
| Taking political sides | Present multiple perspectives |

## References

- See [fable5-prompt-analysis.md](references/fable5-prompt-analysis.md) for the detailed source analysis
- See [preference-application-examples.md](references/preference-application-examples.md) for comprehensive preference application scenarios
- See [memory-system-patterns.md](references/memory-system-patterns.md) for memory system design patterns

## Evaluation & Testability

To build automated QA suites for these principles, use a hybrid approach:
- **Deterministic Tests:** Use assertion checks on execution trajectories (e.g., in LangChain's `AgentAction` log) to verify that `search_web` was called *before* generation for "Auto-Search" compliance.
- **LLM-as-a-Judge:** "Anti-Overformatting" and "The Chef Test" cannot be tested with regex. Use an LLM judge to evaluate the *semantic necessity* of formatting (e.g., "Are these bullets strictly necessary for a procedural step?"), or to detect context bleed (e.g., "Did the agent mention cooking in a Python answer?").
